![IBM Watson Assistant logo]((https://raw.githubusercontent.com/ibm-cloud-docs/images/master/assistant-prod-banner.png))
...
1.  Extract the PPA archive contents:
    ...
    - `{compressed-file-dir}` is the name of the dir that you downloaded {compressed-file-name} to.
...
| `global.image.pullSecret`     | Name of the (manually created) imagePullSecret used to pull docker images. If empty, the default image pull secret for the IBM Cloud Pak for Data internal Docker repository ( sa-{{ .Release.Name }} ) will be used. | `` (empty value) |
