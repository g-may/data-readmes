![IBM Watson Assistant logo](images/assistant-prod-banner.png)
...
This chart deploys a single IBM Watson Assistant slot to accommodate a single IBM Watson Assistant service instance. You choose whether to install a development or production deployment type.
...
- IBM Cloud Private for Data V2.1.0.0
- Kubernetes version 1.10.0 (and newer)
- Helm version 2.9.1
...
### NamespaceLabel Requirements
This chart requires a label to be added to the namespace where IBM Cloud Private for Data is installed (usually zen) prior to installation. To meet this requirement there are cluster scoped pre and post actions that need to occur.

You can add these necessary labels using the supplied instructions/scripts in the pak_extension pre-install directory.

- You can go to `ibm_cloud_pak/pak_extensions/pre-install/clusterAdministration` in your chart directory and run ```./labelNamespace.sh {namespace}``` where `{namespace}` is the namespace where IBM Cloud Private for Data is installed.
...
- Watson Assistant for IBM Cloud Private can run on Intel 64-bit architecture nodes only.
...
Installing the Helm chart deploys a single IBM Watson Assistant instance.
...
1.  To load the file from Passport Advantage into IBM Cloud Private, enter the following command in the IBM Cloud Private command line interface.
    ...
    ```bash
    cloudctl catalog load-archive --registry https://{cluster4d-master-node}:8500/{namespace-name} --archive {compressed_file_name} --repo local-charts
    ```

    - `{compressed_file_name}` is the name of the file that you downloaded from Passport Advantage.
    - `{cluster4d-master-node}` is the IBM Cloud Private for Data cluster master node url.
...
1.  Edit values in the `values.yaml` file.
    ...
    1.  Make a copy of the values.yaml. The `values.yaml` file is stored with the chart.  Rename the file. For example, `my-override.yaml`.

    1.  In your copy of the file, remove all but the configuration settings that you want to replace with your own values.

    1.  Edit the Docker image repository values in the file with values that reflect your environment. At a minimum, you must provide your own values for the following configurable settings:

        - Each image repository value must have the syntax `{icp_url}:8500/{namespace-name}/{unique-path-value}` where `{unique-path-value}` reflects the path from the existing YAML file, such as `icp-wa-tooling-pathed`.
...
| `global.cos.port`        | Only used if global.cos.create is false. Port where COS is listening. Typical port is `443`. | `0` |
| `global.cos.creds.secretName` | If you want custom keys to access the store, create a secret containing `accesskey` (5 - 20 characters), `secretkey` (8 - 40 characters) and `masterkey` (32 bytes long HEX value in the format `KEY_NAME:HEX_VALUE`) in base64 encoding and specify the name for that secret here. If empty, random keys are created in a new secret. | `` (empty value) |
...
For the full list of available AI and Watson services, see the [IBM Cloud catalog](https://cloud.ibm.com/catalog?category=ai) on the public cloud. (**Note**: This link takes you out of IBM Cloud Private to the public IBM Cloud.)
